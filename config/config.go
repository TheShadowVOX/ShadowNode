...
func FromFile(path string) error {
	...
	c.Token = Token{
		ID:    os.Getenv("SHADOWNODE_TOKEN_ID"),
		Token: os.Getenv("SHADOWNODE_TOKEN"),
	}
	if c.Token.ID == "" {
...
func EnableLogRotation() error {
	...
	if _, err := os.Stat("/shadow/node"); err == nil || !os.IsNotExist(err) {
		return err
	}
	...
	t, err := template.New("logrotate").Parse(`{{.LogDirectory}}/shadownode.log {
    size 10M
    compress
    delaycompress
    dateext
    ...
    notifempty
    postrotate
        /usr/bin/systemctl kill -s HUP shadownode.service >/dev/null 2>&1 || true
    endscript
}`)
	if err != nil {
		return err
