...
// The time at which ShadowNode was booted. No JWT's created before this time are allowed to
// connect to the socket since they may have been marked as denied already and therefore
// could be invalid at this point.
//
// By doing this we make it so that a user who gets disconnected from ShadowNode due to a ShadowNode
// reboot just needs to request a new token as if their old token had expired naturally.
var shadownodeBootTime = time.Now()
...
// before ShadowNode was booted, or because we have denied all tokens with the same JTI
...
func (p *WebsocketPayload) Denylisted() bool {
	...
	// If the time that the token was issued is before the time at which ShadowNode was booted
	// then the token is invalid for our purposes, even if the token "has permission".
	if p.IssuedAt.Time.Before(shadownodeBootTime) {
		return true
	}
