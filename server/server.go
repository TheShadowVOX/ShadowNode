...
import (
	...
	"github.com/TheShadowVOX/shadownode/events"
	"github.com/TheShadowVOX/shadownode/remote"
	"github.com/TheShadowVOX/shadownode/server/filesystem"
...
type Server struct {
	...
	// fetched from the ShadowNode Server instance each time the server process is
...
// Sync syncs the state of the server on the Panel with ShadowNode. This ensures that
// we're always using the state of the server from the Panel and allows us to
// not require successful API calls to ShadowNode to do things.
//
// This also means mass actions can be performed against servers on the Panel
// and they will automatically sync with ShadowNode when the server is started.
...
// instance on ShadowNode. This includes the information needed by the Panel in order
